    ...
    persist(
        (set, get) => ({
            ...
            getTopicProgress: (topicId) => {
                ...
                // Direct questions on topic
                const directQuestions = (topic.questionIds || []).map(id => state.questions.byId[id]).filter(Boolean);
                // Sub-topic questions
                const subTopicQuestions = (topic.subTopicIds || []).flatMap(stId => {
                    const st = state.subTopics?.byId?.[stId];
                    if (!st) return [];
                    return (st.questionIds || []).map(id => state.questions.byId[id]).filter(Boolean);
                });
                const allQuestions = [...directQuestions, ...subTopicQuestions];
            ...
            deleteQuestion: (id) => set((state) => {
                ...
                // Handle sub-topic: remove question from sub-topic's questionIds
                let updatedSubTopics = state.subTopics;
                if (question.subTopicId) {
                    const subTopic = state.subTopics?.byId?.[question.subTopicId];
                    if (subTopic) {
                        updatedSubTopics = {
                            ...state.subTopics,
                            byId: {
                                ...
                                [question.subTopicId]: {
                                    ...subTopic,
                                    questionIds: (subTopic.questionIds || []).filter(qId => qId !== id)
                                }
                            }
                        };
                    }
